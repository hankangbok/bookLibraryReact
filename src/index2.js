  ...
  render() {
    ...
    return (
      <div className="bookContainer">
        ...
        <br></br>
        <RemoveBookFromLibrary title={title} />
      </div>
    );
...
class RemoveBookFromLibrary extends React.Component {
  ...
  handleClick(e) {
    console.log("remove the book with title "+this.props.title);
    const index = BOOKSLIST.findIndex(eachBook => eachBook.title === this.props.title);
    console.log(index);
    // BOOKSLIST = BOOKSLIST.splice(index);
    // let BOOKSLISTNew = BOOKSLIST.splice(index);
    console.log(BOOKSLIST.indexOf(this.props.title));
    let BOOKSLISTNew = BOOKSLIST.splice(BOOKSLIST);
  }
  render() {
